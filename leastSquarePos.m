function [pos,el, az, dop, raim] = leastSquarePos(satpos,obs,settings,cno)
...
for iter = 1:nmbOfIterations
    ...
    for i = 1:nmbOfSatellites
        if iter == 1
            ...
            constant_a = 0.003; % 3 mm for general receiver --sbs
            if el(i)>=30
                weight(i) = constant_a.^2;
            else
                weight(i) = constant_a.^2/(4*(sin(el(i) * dtr )^2));
            end

            % using CN0 based weighting --sbs
            % constant_a = 2;
            % cn0_ref = max(cn0);
            % weight(i) = (cn0(i)./cn0_ref).^constant_a;
